...
export function getNatureResult({ nature, pokemonStats }) {
    const stats = getPokemonStatsAndRank({ pokemonStats })
    const NatureCategoryArray = {0: "Neutra", 2: "Ruim", 4: "Mediana", 6:"Boa", 8: "Ótima", 10: "Perfeita"}
    console.log(nature)
...
export function getIvNote({ ivs, pokemonStats }) {
    const stats = getPokemonStatsAndRank({ pokemonStats: pokemonStats });
    let sum = 0;
    const a = [];
    for (let i = 0; i < 6; i++) {
...
export function getPokemonStatsAndRank({ pokemonStats }) {
